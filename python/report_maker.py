...
class ReportSubcomponent:
    ...
    def find_username_token_using_regex(df:pd.DataFrame, token:str):
        """
    ...
    def find_username_email_using_regex(input_str):
        """
        ...
        Returns:
            pd.DataFrame: DataFrame with the found username
        """
        # Example regex pattern to find username
        pattern = r'(?P<username>[a-zA-Z0-9_]+)'
        matches = input_str.extract(pattern)
        return matches
...
class_init.find_username_email_using_regex(input_str=" ".join(df['username']))
